...
uid_min() {
    awk '/^[[:space:]]*UID_MIN/ {print $2}' "$(hk_path /etc/login.defs)" 2>/dev/null || echo 1000
}
...
check_inactive_users() {
    ...
    local min_uid max_days users
    min_uid = "$(uid_min)"
    max_days = "${MAX_INACTIVE_DAYS:-90}"

    users="$(
    awk -F: -v min="$min_uid" '
        $3 >= min && $7 !~ /(nologin|false)$/ {print $1}
    ' "$passwd_file"
    )"
